...
static const NSInteger tabItemHeight = 135;
#define tabsButtonsFrame CGRectMake(0, 10 + iOS_7 * 20, tabBarWidth, self.view.bounds.size.height)
    ...
    if (tabs) {
        ...
        @strongify(self)
            if ([obj isKindOfClass:[SMTabBarItem class]]) {
                self->_tabsButtonsHeight += tabItemHeight;
    ...
    if (actions) {
        ...
        [actions enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if ([obj isKindOfClass:[SMTabBarItem class]]) {
                ...
                self->_actionsButtonsHeight += tabItemHeight;
                [tmpItems addObject:obj];
            }
        }];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (row == 0) {
        CGFloat heigth = (SCREEN_HEIGHT - tabItemHeight*[_tabsButtons count])/2.0;
        return heigth;
    }
