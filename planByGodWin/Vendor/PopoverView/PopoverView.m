...
+ (PopoverView *)showPopoverAtPoint:(CGPoint)point withArrowDirection:(BOOL)up inView:(UIView *)view withContentView:(UIView *)cView delegate:(id<PopoverViewDelegate>)delegate{
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    popoverView.above = up;
    [popoverView showAtPoint:point inView:view withContentView:cView];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withContentView:(UIView *)cView {
    ...
    topView = [[keyWindow subviews] lastObject];
