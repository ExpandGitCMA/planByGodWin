...
//	DEBUG_NSLog(@"frame = %d", nFrameCount);
	Decoder_Interface_exit(destate);
...
//    DEBUG_NSLog(@"documentPath=%@", documentPath);
...
void WriteWavFileHeader(FILE* fpwave, int nFrame)
{
	char tag[10] = "";
...
// 将AMR文件解码成WAVE文件
int DecodeAmrFileToWavFile(const char* pchAmrFileName, const char* pchWavFileName)
{
#ifdef DEBUG
    static int  count = 0;
    count++;
	...
	// 创建并初始化WAVE文件
	fpwave = fopen(pchWavFileName, "wb");
	if (fpwave == NULL)
    {
        if (fpamr != NULL)
        {
	...
	WriteWavFileHeader(fpwave, nFrameCount);
