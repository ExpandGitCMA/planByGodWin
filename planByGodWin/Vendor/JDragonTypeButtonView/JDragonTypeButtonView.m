...
-(void)setTypeButtonTitles:(NSArray*)titles  withDownLableHeight:(CGFloat)downLabHeight andDeleagte:(id<JDragonTypeButtonActionDelegate>)deleget {
    // modify by hmy
    [self setTypeButtonTitles:titles
       buttonNormalImageNames:nil
     buttonSelectedImageNames:nil
          withDownLableHeight:downLabHeight
                  andDeleagte:deleget
                         font:nil
                        color:nil];
}
...
// add by hmy begin
-(void)setTypeButtonTitles:(NSArray*)titles
    buttonNormalImageNames:(NSArray<NSString *> *)normalImageNames
  buttonSelectedImageNames:(NSArray<NSString *> *)selectedImageNames
       withDownLableHeight:(CGFloat)downLabHeight
               andDeleagte:(id<JDragonTypeButtonActionDelegate>)deleget
                      font:(UIFont *)font
                     color:(UIColor *)color {
    [self customInit];
    ...
    for (int  i=0; i<titles.count; i++)
    {
        ...
        if (normalImageNames.count > i) {
            [typeBtn setImage:[UIImage imageNamed:normalImageNames[i]] forState:UIControlStateNormal];
            typeBtn.imageEdgeInsets = UIEdgeInsetsMake(0, -20, 0, 0);
        }
        
        if (selectedImageNames.count > i) {
            [typeBtn setImage:[UIImage imageNamed:selectedImageNames[i]] forState:UIControlStateSelected];
            typeBtn.imageEdgeInsets = UIEdgeInsetsMake(0, -20, 0, 0);
        }
...
-(void)setSelectTypeIndex:(NSInteger)index
{
    ...
    if ([self.delegate respondsToSelector:@selector(didClickTypeButtonAction:withIndex:)]) {
        [self.delegate didClickTypeButtonAction:nil withIndex:index];
    }
}
/**
 *  设置选中button
