...
//    content.badge = [NSNumber numberWithInteger:count];
    ...
    if (type == DFCMessageObjectCountTypeSingle) {
        if ([DFCUserDefaultManager getSingleMessageSoundSetting]) {
            content.sound = sound;
        } else {
            content.sound = nil;
        }
    } else {
        if (type == DFCMessageObjectCountTypeMultiple) {
            if ([DFCUserDefaultManager getMutipleMessageSoundSetting]) {
                content.sound = sound;
            } else {
                content.sound = nil;
            }
