...
+ (UIImage *)noPrejudiceScreenShootForView:(UIView *)view {
    @autoreleasepool {
        ...
        [view.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[DFCVideoView class]]) {
                DFCVideoView *videoView = (DFCVideoView *)obj;
                [videoView getScreenThumbImage];
            }
        }];
...
//        }
        ...
        UIImage *smallSizeImage = [UIImage compressPngImage:image targetSize:imageSize];
        //UIImageWriteToSavedPhotosAlbum(smallSizeImage, nil, nil, nil);
...
+ (UIImage *)screenShootForBoard:(DFCBoard *)board {
    @autoreleasepool {
        CGSize imageSize = [board bounds].size;
        UIGraphicsBeginImageContextWithOptions(imageSize, NO, 0);
        CGContextRef context = UIGraphicsGetCurrentContext();
        ...
        for (UIView *subview in board.subviews) {
            [subview removeFromSuperview];
        }
        
        board.layer.contents = nil;
