...
@interface DFCDBModel () {
    
}

@end

@implementation DFCDBModel
...
+ (BOOL)createTable {
    ...
    DEBUG_NSLog(@"%@", [DFCDBHelp dbPath]);
    DFCDBHelp *dbHelp = [DFCDBHelp sharedInstance];
...
+ (NSDictionary *)params:(DFCDBModel *)model {
...
+ (BOOL)saveObjects:(NSArray *)array {
    ...
    [dbHelp.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (DFCDBModel *model in insertArray) {
...
+ (NSArray *)findByConditionStatement:(NSString *)statement {
    ...
    [dbHelp.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            DFCDBModel *model = [[self.class alloc] init];
            for (int i = 0; i < model.columnNames.count; i++) {
